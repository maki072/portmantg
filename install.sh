...
if [ -f "$SERVICE_FILE" ]; then
  ...
  warn "Your current settings (admin password, turnstile-secret, etc.) will be shown below."
  echo
  grep "ExecStart" "$SERVICE_FILE" | sed 's/ *-/\n  -/g' | grep -v "^$" || true
  echo
  prompt OVERWRITE_SERVICE "Overwrite service file with new settings? [y/N]" "n"
  [[ "${OVERWRITE_SERVICE,,}" =~ ^y ]] || WRITE_SERVICE=0
fi
...
EOF
  ...
  info "Keeping existing service file (binary updated, settings preserved)."
fi
...
configure_nginx_angie() {
  ...
  if [ -f "$conf_file" ]; then
    warn "Web server config already exists: $conf_file — skipping (not overwritten)."
    return
  fi
